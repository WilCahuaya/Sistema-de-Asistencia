-- ============================================
-- RPC: Días incompletos por aula (para reportes)
-- Detecta fechas donde hay asistencias registradas pero no todos los estudiantes
-- del aula tienen registro. Lógica única en BD para evitar desincronización.
-- Usa SECURITY DEFINER para que facilitadores puedan leer.
-- ============================================
...
RETURNS TABLE(
  ...
  -- Total de estudiantes por (aula, fecha) según estudiante_periodos
  totales AS (
    SELECT 
      c.aula_id,
      c.fecha,
  ...
  -- Estudiantes que debían estar en cada (aula, fecha)
  ...
  -- Marcados: asistencias de esos estudiantes en esa fecha (fcp_id para filtrar)
  marcados AS (
    SELECT 
      epf.aula_id,
      epf.fecha,
...
COMMENT ON FUNCTION public.dias_incompletos_por_aula IS 'Devuelve días incompletos: fechas donde hay asistencias pero no todos los estudiantes del aula tienen registro. Usado en reportes. SECURITY DEFINER para facilitadores.';
